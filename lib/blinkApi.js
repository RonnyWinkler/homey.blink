...
class blinkApi {    
    ...
    enableCameraMotion(id) {
        return new Promise((resolve, reject) => {
            ...
            this.getCamera(id).then(camera => {
                ...
                if (cameraType == 3){
                    endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + systemId + "/doorbells/" + id + "/config";
                    payload = {
                        "enabled": true
                    }
                }
    ...
    setCameraLight(cameraId, floodlightId, on) {
        return new Promise((resolve, reject) => {
            ...
            this.getCamera(cameraId).then(camera => {
                ...
                let payload = { };
                let endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + systemId + "/cameras/" + cameraId + "/accessories/storm/" + floodlightId + "/lights";
                if (on){
    ...
    requestCameraVideo(id) {
        return new Promise((resolve, reject) => {
            ...
            this.getCamera(id).then(camera => {
                ...
                if (cameraType == 3){
                    endpoint = "/api/v1/accounts/" + this._account.account_id +"/networks/" + systemId + "/doorbells/" + id + "/clip";
                }
                this._post(endpoint, payload).then(response => {
                    const result = JSON.parse(response);
                    if (result == null) {
    ...
    captureSnapshot(cameraID) {
        return new Promise((resolve, reject) => {
            ...
            this.getCamera(cameraID).then(camera => {
                ...
                if (camera.type == 3){
                    endpoint = "/api/v1/accounts/" + this._account.account_id +"/networks/" + networkID + "/doorbells/" + cameraID + "/thumbnail";
                }
                this._post(endpoint, payload).then(response => {
                    const result = JSON.parse(response);
                    if (result == null) {
    ...
    getCameraVideoStreamLocal(videoId) {
        return new Promise((resolve, reject) => {
            ...
            let endpoint = '';
            endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + videoId.networkId  + "/sync_modules/" + videoId.syncmoduleId + "/local_storage/manifest/request";
            const payload = '';
            this._post(endpoint, payload).then(response => {
                const result = JSON.parse(response);
                if (result == null) {
                    ...
                    let endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + videoId.networkId  + "/sync_modules/" + videoId.syncmoduleId + "/local_storage/manifest/request/" + requestId;
                    ...
                    this.sleep( 1000 ).then(sleep => {
                        this._get(endpoint, null, false, [400]).then(response => {
                            ...
                            if (result == null) {
                                ...
                                let endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + videoId.networkId  + "/sync_modules/" + videoId.syncmoduleId + "/local_storage/manifest/" + manifestId + "/clip/request/" + videoId.id;
                                
                                this.sleep( 1000 ).then(sleep => {
                                    const payload = '';
                                    this._post(endpoint, payload, false).then(response => {
                                        const result = JSON.parse(response);
                                        if (result == null) {
                                            reject("Error during deserialization: " + response);
                                        } 
                                        else {
                                            let endpoint = "/api/v1/accounts/" + this._account.account_id + "/networks/" + videoId.networkId  + "/sync_modules/" + videoId.syncmoduleId + "/local_storage/manifest/" + videoId.manifestId + "/clip/request/" + videoId.id;
