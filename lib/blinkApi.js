...
class blinkApi {    
    ...
    //             this._accountId = result.account;
    ...
    //             pin: pinCode,
    //         }
    //         this._post(`/api/v4/account/${this._accountId}/client/${this._clientId}/pin/verify`, payload, true, true).then(response => {
    //             const result = JSON.parse(response);
    //            if (result.valid) {
    ...
    async _post(endpoint, payload) {
        // return new Promise((resolve, reject) => {
            ...
            return await this._http('POST', this.getRegionalUrl(), endpoint, payload, headers);
    ...
    async _get(endpoint, payload, ignoreHttpErrors=[]) {
            ...
            try{
                return await this._http('GET', this.getRegionalUrl(), `${endpoint}${this._toQueryString(payload)}`, '', headers);
            }
            catch (error){
